...
if(!isset($_SESSION['sukses_login'])){$_SESSION['sukses_login']=""; } //jika variabel belu      //mulai session, krena kita akan menggunakan session pd file php ini
    ...
    //apabila session ini sudah ada .. lanjut ke dashboard
       header("Location:mlebu_pelamar.php");
}
      include "koneksi.php";
      ...
      $password = $_POST['password'];
        ...
        $query = mysql_query("select * from pelamar where email='$username' AND password='$password'") or die(mysql_error());//menjalankan perintah query
        ...
        $rows = mysql_num_rows($query); //menghitung baris
            if ($rows == TRUE) {
                ...
                header("location:mlebu_pelamar.php");     // dan alihkan ke index.php
                } else{
                    if((empty($username)) and (empty($password))){  
                        echo $_SESSION['sukses_login']; 
                    }
                    elseif (!(empty($username)) and !(empty($password))){
                        echo "<script>alert('Masukkan Username dan Password yang benar!'); window.location = 'login_pelamar.php'</script>";
            ...
            ?>
             <form action="" method="post">
              <div class="form-group">
                ...
                <center><a href="register_pelamar.php">Register</a></center>
